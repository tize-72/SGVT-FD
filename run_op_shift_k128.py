"""
Operating Condition Shift — K=128, 100 epochs.
"""
                ...
                for f in sorted(os.listdir(size_path)):
                    if f.endswith('.mat'):
                        ...
                        try:
                            ...
                            pass
    total = 0
    for ld, items in load_data.items():
        print(f"  Load {ld}HP: {len(items)} segments")
        total += len(items)
...
def run():
    print("=" * 70)
    print("Op Shift — K=128, 100 epochs")
    print("=" * 70)
    ...
    for exp_name, exp_cfg in experiments.items():
        ...
        for seed in seeds:
            ...
            print(f"  SGVT-FD K=128 (seed={seed})...")
            model = SGVTModel(num_classes=4, num_groups=128, use_domain_prior=False,
                              use_mi_loss=True, device=DEVICE).to(DEVICE)
            metrics, params = train_model(model, train_loader, val_loader, test_loader,
                                          n_epochs=100, model_name=f"{exp_name}_sgvt_k128_seed{seed}")
            all_results[f"{exp_name}_sgvt_k128_seed{seed}"] = {
                'accuracy': metrics['accuracy'], 'f1': metrics['f1'], 'params': params
            }
            print(f"    Acc: {metrics['accuracy']*100:.2f}%, F1: {metrics['f1']*100:.2f}%")
    out_path = os.path.join(OUTPUT_DIR, "op_shift_k128_results.json")
    ...
    for exp_name in experiments:
        accs = []
        for seed in seeds:
            key = f"{exp_name}_sgvt_k128_seed{seed}"
