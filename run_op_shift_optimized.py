"""
Operating Condition Shift Experiment — Optimized.
K=64, 100 epochs to close the gap with No Grouping.
"""
...
from src.models.baselines import CVTBaseline
...
def train_model(model, train_loader, val_loader, test_loader, n_epochs=100, model_name="model"):
    ...
    best_val_acc = 0
    out_dir = os.path.join(OUTPUT_DIR, "optimized", model_name)
...
def run_op_shift_optimized():
    """Run optimized op_shift with K=64."""
    print("=" * 70)
    print("Operating Condition Shift — Optimized (K=64, 100 epochs)")
    print("=" * 70)
    ...
    for exp_name, exp_cfg in experiments.items():
        ...
        print(f"  Train: {len(train_items)} segments, Test: {len(test_items)} segments")
        ...
        for seed in seeds:
            ...
            # SGVT-FD with K=64
            print(f"  SGVT-FD K=64 (seed={seed})...")
            sgvt = SGVTModel(num_classes=4, num_groups=64, use_domain_prior=False,
                              use_mi_loss=True, device=DEVICE).to(DEVICE)
            metrics, params = train_model(sgvt, train_loader, val_loader, test_loader,
                                          n_epochs=100, model_name=f"{exp_name}_sgvt_k64_seed{seed}")
            all_results[f"{exp_name}_sgvt_k64_seed{seed}"] = {
                'accuracy': metrics['accuracy'], 'f1': metrics['f1'], 'params': params
            }
            print(f"    Acc: {metrics['accuracy']*100:.2f}%, F1: {metrics['f1']*100:.2f}%")

    # Save results
    out_path = os.path.join(OUTPUT_DIR, "op_shift_optimized_results.json")
    ...
    for exp_name in experiments:
        accs = []
        for seed in seeds:
            key = f"{exp_name}_sgvt_k64_seed{seed}"
            if key in all_results:
                accs.append(all_results[key]['accuracy'] * 100)
        if accs:
            print(f"  {exp_name} SGVT-FD K=64: {np.mean(accs):.2f}% ± {np.std(accs):.2f}%")
